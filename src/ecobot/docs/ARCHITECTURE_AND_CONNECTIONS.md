...
- `/ecobot/plant_scan_cmd`: Published by the ecobot-ui dashboard over rosbridge (when a user clicks UI buttons).
- `/ecobot/waypoints`: Published by the ecobot-ui dashboard over rosbridge.
...
- `/ecobot/plant_scan_status`: Read by the ecobot-ui dashboard over rosbridge to show the current mission state.
- `/ecobot/scan_capture`: Read by the ecobot-ui dashboard over rosbridge to display the AI's health assessment of the plant.
...
2. **Dashboard (ecobot-ui via rosbridge):** Receives the click and publishes `{"data": "start"}` to `/ecobot/plant_scan_cmd`.
